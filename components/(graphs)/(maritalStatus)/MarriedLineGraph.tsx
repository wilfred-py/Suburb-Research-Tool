...
export default function MarriedLineGraph(props: MarriedProps) {
    ...
    async function fetchMarriedDataByYear(year: string, tableName: string, selectedSuburb: string | null) {
        try {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            setSuburbMarried([null, null, null, null, null]);
            setStateMarried([null, null, null, null, null]);
            setAustraliaMarried([null, null, null, null, null]);

            // console.log(`selectedSuburb: ${props.selectedSuburb}`);
            const years = ["2011", "2016", "2021"];
            const dataPromises = years.map((year) => fetchMarriedDataByYear(year, `data_${year}`, props.selectedSuburb));

            // Reset insufficient useState to false
            setInsufficientSuburbData(false);

            const newSuburbMarried = [...suburbMarried];
            const newStateMarried = [...stateMarried];
            const newAustraliaMarried = [...australiaMarried];
            ...
            validData.forEach((result) => {
                ...
                try {
                    ...
                    // * 2011
                    if (year == "2011") {
                        const suburbMarried = data[0]["percentage_married_in_suburb"];
                        const stateMarried = data[0]["percentage_married_in_state"];
                        const australiaMarried = data[0]["percentage_married_in_australia"];

                        // >> Suburb
                        if (suburbMarried) {
                            newSuburbMarried[2] = suburbMarried;
                            setSuburbMarried(newSuburbMarried);
                        } else if (!suburbMarried) {
                            // ! If data is not available, remove element from array so line graph does not display it
                            newSuburbMarried[2] = null;
                            setSuburbMarried(newSuburbMarried);
                        }

                        // >> State
                        if (stateMarried) {
                            newStateMarried[2] = stateMarried;
                            setStateMarried(newStateMarried);
                        } else if (!stateMarried) {
                            // ! If data is not available, remove element from array so line graph does not display it
                            newStateMarried[2] = null;
                            setStateMarried(newStateMarried);
                        }

                        // >> Australia
                        if (australiaMarried) {
                            newAustraliaMarried[2] = australiaMarried;
                            setAustraliaMarried(newAustraliaMarried);
                        } else if (!australiaMarried) {
                            // ! If data is not available, remove element from array so line graph does not display it
                            newAustraliaMarried[2] = null;
                            setAustraliaMarried(newAustraliaMarried);
                        }
                    }
                    ...
                    // * 2016
                        ...
                        // >> Suburb
                        if (suburbMarried) {
                            newSuburbMarried[3] = suburbMarried;
                            setSuburbMarried(newSuburbMarried);
                        } else if (!suburbMarried) {
                            // ! If data is not available, remove element from array so line graph does not display it
                            newSuburbMarried[3] = null;
                            setSuburbMarried(newSuburbMarried);
                        }

                        // >> State
                        if (stateMarried) {
                            newStateMarried[3] = stateMarried;
                            setStateMarried(newStateMarried);
                        } else if (!stateMarried) {
                            // ! If data is not available, remove element from array so line graph does not display it
                            newStateMarried[3] = null;
                            setStateMarried(newStateMarried);
                        }

                        // >> Australia
                        if (australiaMarried) {
                            newAustraliaMarried[3] = australiaMarried;
                            setAustraliaMarried(newAustraliaMarried);
                        } else if (!australiaMarried) {
                            // ! If data is not available, remove element from array so line graph does not display it
                            newAustraliaMarried[3] = null;
                            setAustraliaMarried(newAustraliaMarried);
                        }
                    }
                    ...
                    // * 2021
                        ...
                        // >> Suburb
                        if (suburbMarried) {
                            newSuburbMarried[4] = suburbMarried;
                            setSuburbMarried(newSuburbMarried);
                        } else if (!suburbMarried) {
                            // ! If data is not available, remove element from array so line graph does not display it
                            newSuburbMarried[4] = null;
                            setSuburbMarried(newSuburbMarried);
                        }

                        // >> State
                        if (stateMarried) {
                            newStateMarried[4] = stateMarried;
                            setStateMarried(newStateMarried);
                        } else if (!stateMarried) {
                            // ! If data is not available, remove element from array so line graph does not display it
                            newStateMarried[4] = null;
                            setStateMarried(newStateMarried);
                        }

                        // >> Australia
                        if (australiaMarried) {
                            newAustraliaMarried[4] = australiaMarried;
                            setAustraliaMarried(newAustraliaMarried);
                        } else if (!australiaMarried) {
                            // ! If data is not available, remove element from array so line graph does not display it
                            newAustraliaMarried[4] = null;
                            setAustraliaMarried(newAustraliaMarried);
                        }
                    }
                } catch (error) {
                    ...
                    if (year === "2001") {
                        ...
                        checkIfInsufficientData(suburbMarried);
                    }
                }
            });
        }
    ...
    const data = [
        { name: "2011", Suburb: suburbMarried[2], State: stateMarried[2], Australia: australiaMarried[2] },
        { name: "2016", Suburb: suburbMarried[3], State: stateMarried[3], Australia: australiaMarried[3] },
        { name: "2021", Suburb: suburbMarried[4], State: stateMarried[4], Australia: australiaMarried[4] },
    ];
    ...
    return (
        <div>
            <div className="flex flex-col justify-center">
                <h1 className="mt-4 mb-4 text-lg text-center font-bold">Married</h1>
