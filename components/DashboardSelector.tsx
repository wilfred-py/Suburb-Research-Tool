...
export default function DashboardSelector(props: DashboardSelectorProps) {
    ...
    const getButtonCSS = (viewName: string) => {
        if (viewName === props.selectedView) {
            return `relative focus:outline-none focus:ring-0 whitespace-nowrap mr-2 md:mr-0 md:ml-2.5 p-2 py-1 md:py-2 bg-hoverBlue hover:bg-hoverBlue text-left leading-6 text-black font-normal border-1 border-navigationBarBlue rounded rounded-sm hover:cursor-pointer select-none`;
        } else {
            return `relative focus:outline-none focus:ring-0 whitespace-nowrap mr-2 md:mr-0 md:ml-2.5 p-2 py-1 md:py-2 hover:bg-hoverBlue text-left hover:text-gray-900 leading-6 text-black font-normal border-1 border-navigationBarBlue rounded rounded-sm hover:cursor-pointer select-none`;
        }
    };
